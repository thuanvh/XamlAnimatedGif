...
namespace XamlAnimatedGif.Decoding
{
    internal abstract class GifExtension : GifBlock
    {
        ...
        internal new static GifExtension Read(Stream stream, IEnumerable<GifExtension> controlExtensions)
        {
            ...
            return label switch
            {
                GifGraphicControlExtension.ExtensionLabel => GifGraphicControlExtension.Read(stream),
                GifCommentExtension.ExtensionLabel => GifCommentExtension.Read(stream),
                GifPlainTextExtension.ExtensionLabel => GifPlainTextExtension.Read(stream, controlExtensions),
                GifApplicationExtension.ExtensionLabel => GifApplicationExtension.Read(stream),
                _ => throw GifHelpers.UnknownExtensionTypeException(label),
            };
        }
    }
